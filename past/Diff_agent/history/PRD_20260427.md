...
- **G3**：**觸發**動作完全在 Slack（上傳音檔、收到通知、查狀態）；**review** 動作切換到 Google Docs Suggesting mode。Slack ↔ Docs 之間透過 deep-link 一鍵跳轉。
...
>-  **Kickoff 日**：2026-04-27 (Mon)
>-  **PoC 完成 / BA 試用**：2026-05-08 (Fri)（第 2 週末）
>- **總工時**：10 個工作日
...
#### Week 1 (04/27 – 05/01)：LangGraph 骨架 + Docs API 串接 + Slack entry point

| Day | Date | 任務 | 確認 |
|---|---|---|---|
| **D1** | Mon 04/27 | Kickoff (30min) → ASR spike (Gemini audio) + LangGraph 環境裝起來 + Docs API spike（讀取 sample Doc，依 §3.3 模板辨識 H1/H2）+ OAuth flow 本機跑通 | `transcribe` node + Docs API `documents.get` 都能單獨跑出結果 |
| **D2** | Tue 04/28 | 定義 `BRDAgentState`、寫 `fetch_doc` node（heading 樣式偵測、空 heading 過濾、表格 / 圖片以 placeholder 標示、序列化成 LLM 輸入格式）+ `extract_edits` node（LLM structured output 含 `doc_id` + `section_heading` + `anchor_text`，`target_kind` 固定 `paragraph`） | `graph.invoke()` 從 mp3 + Doc URL 拿到 `proposals` |
| **D3** | Wed 04/29 | 寫 `apply_as_suggestions` node：每條 edit 計算 Docs API `startIndex` / `endIndex`（從 anchor_text 反推）、組 `batchUpdate` 寫成 Suggesting mode + `comments.create` 加 anchored comment | CLI 一條龍：mp3 + Doc URL → 真實 Doc 上看到 N 條建議 + comments |
| **D4** | Thu 04/30 | Slack bot 骨架 (slack-bolt)、slash command 收 Doc URL、檔案上傳處理、OAuth flow 與 Slack 整合（首次使用跳 consent URL）| `/brd-update <doc-url>` + 上傳 mp3 → bot 回「處理中」 |
| **D5** | Fri 05/01 | Slack bot 接 graph 完成事件 + 摘要訊息 + 「Open in Google Docs」按鈕；MemorySaver 跑通 end-to-end | **Demo #1：BA 用 Slack 觸發 → 在 Google Docs 看到建議** |

#### Week 2 (05/04 – 05/08)：Heading 對齊強化 + Suggestion 寫入精度 + 內測

| Day | Date | 任務 | 確認 |
|---|---|---|---|
| **D6** | Mon 05/04 | 強化 `fetch_doc`：heading 樣式辨識魯棒化（過濾空 heading / 重複 heading 文字 / 樣式套用不一致案例）、Doc 序列化格式優化（讓 LLM 更好讀；表格 / 圖片以結構化 placeholder 註記）| 對 §3.3 標準模板 sample Doc，所有 H1/H2 章節 ID 正確生成；異常 heading 不污染 ID 集合 |
| **D7** | Tue 05/05 | `apply_as_suggestions` 精度強化：anchor_text ambiguous 時的 fallback、跨段落 edit、insert 位置計算（Heading 後 vs 段落間）、回寫 `suggestion_id` 到 Postgres | 5 條樣本 edit 全部寫對位置（無 ambiguous match 失敗） |
| **D8** | Wed 05/06 | 加 Drive Comments API 讀取上輪 reject reasons（餵給下輪 `extract_edits`）+ PostgresSaver 換掉 MemorySaver + OAuth refresh token 自動 refresh | 第二次跑同一 Doc 會避開上次被 reject 的提議 |
| **D9** | Thu 05/07 | Basic PII masking (regex: 姓名 / 身分證 / 卡號 / 電話) + audit log + 真實會議測試 (≥ 2 場) | 內測通過，ground truth Suggestion Accept Rate ≥ 50% |
| **D10** | Fri 05/08 | Bug fix + README + handover 文件 + final demo | **Demo #2：BA 正式試用 + retro** |
